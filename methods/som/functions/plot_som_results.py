...
def initialize_plot(args):
    ...
    return [geo_data, geo_headers, som_data, som_table, som_headers, som_dict, grid, annot_ticks, outgeofile, clusters]
...
"""
def draw_som_clusters(som_data, som_table, annot_ticks, som_headers, outgeofile, clusters, grid_type):    
    if(grid_type.lower()=="rectangular"):
        mpl.rcParams.update({'font.size': 14})  
        for i in range(0,len(som_data)): 
