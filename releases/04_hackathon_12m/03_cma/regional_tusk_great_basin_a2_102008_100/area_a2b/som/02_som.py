...
def main(args):
    ...
    model = hack_12m_tusk.regional_scale_great_basin[MODEL]

    BASE_PATH = files("beak.data")

    # Choose data path
    ROOT_PATH = BASE_PATH / "PROCESSED" / "regional_tusk_great_basin_a2b_102008_100"

    # Choose data paths
    PATH_STD_DATA = ROOT_PATH / "unified_imputed_scaled_std"
    PATH_SPECTRAL_DATA = ROOT_PATH / "unified" / "remote_sensing" / "spectral"
    PATH_LABELS = ROOT_PATH / "labels" / "ta2_usgs_0822a.tif"
    #PATH_LABELS = ROOT_PATH / "labels" / "gbwdb_rock_w_ppm_reclass9.tif"

    model_dict, file_list, counts = load_model(
        model=model,
        folders=[PATH_STD_DATA, PATH_SPECTRAL_DATA],
...
if __name__ == "__main__":
    ...
    parser.add_argument("--epochs", type=int, default=10)                   # Number of epochs to train the map for
    ...
    parser.add_argument("--radius0", type=float, default=0)                 # The initial radius on the map where the update happens around a best matching unit. Default value of 0 will trigger a value of min(n_columns, n_rows)/2.
    parser.add_argument("--radiusN", type=float, default=1)                 # The radius on the map where the update happens around a best matching unit in the final epoch. Default: 1.
    parser.add_argument("--radiuscooling", type=str, default="linear")      # The cooling strategy between radius0 and radiusN
    parser.add_argument("--scalecooling", type=str, default="linear")       # The cooling strategy between scale0 and scaleN
    parser.add_argument("--scale0", type=float, default=0.1)                # The initial learning scale. Default value: 0.1.
    parser.add_argument("--scaleN", type=float, default=0.01)               # The learning scale in the final epoch. Default: 0.01.
