...
def cluster_hit_count(som, output_file_somspace, output_folder):
	...
	Args:
		...
		output_folder (str): file path for writhing txt file
	"""
	...
	np.savetxt(output_folder+"/cluster_hit_count.txt", result_array, fmt='%d', delimiter='\t', header='ClusterNumber\tHitCount', comments='')

def create_bmu_id(som_x, som_y, output_folder):
	"""
	Generate BMU (Best Matching Unit) IDs for all combinations of SOM (Self-Organizing Map) coordinates.
	Args:
	som_x (int): The extent of the SOM space along the x-axis.
	som_y (int): The extent of the SOM space along the y-axis.
	Returns:
	list: A 2D array of BMU IDs for each combination of SOM coordinates.
	"""
	bmu_ids = []
	for y in range(som_y):
		row = []
		for x in range(som_x):
			# Generate a unique BMU ID for each combination of som_x and som_y
			bmu_id = y * som_x + x
			row.append(bmu_id)
		bmu_ids.append(row)
	
	# write bmu_id to txt file:
	file_path = output_folder + "/bmu_ids.txt"
